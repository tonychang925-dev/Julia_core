...
class EdgeTTSPCMProvider:
    ...
    async def stream_to_track(self, text: str, track, generation: int) -> bool:
        ...
        try:
            ...
            # Pass loop explicitly — thread executor has no event loop
            result = await loop.run_in_executor(
                None, self._decode_and_enqueue, tmp_path, track, generation, self._cancel, loop
            )
            Path(tmp_path).unlink(missing_ok=True)
            return result
    ...
    def _decode_and_enqueue(self, mp3_path: str, track, generation: int,
                            cancel: asyncio.Event, loop: asyncio.AbstractEventLoop) -> bool:
        """Synchronous: decode MP3, resample, packetize, enqueue via main loop."""
        try:
            import av
            import numpy as np
            ...
            for packet in container.demux(audio_stream):
                ...
                # Decode packet → AudioFrame(s), then resample
                for raw_frame in audio_stream.decode(packet):
                    if cancel.is_set():
                        return False
