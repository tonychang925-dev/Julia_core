...
class TTSAudioTrack(MediaStreamTrack):
    ...
    async def wait_generation_consumed(self, generation: int, timeout: float = 10.0) -> bool:
        """Wait until all frames of `generation` have been consumed by recv().

        Returns True if drained, False if timeout or generation changed.
        """
        import asyncio as _asyncio
        deadline = _asyncio.get_event_loop().time() + timeout
        while self._active_generation == generation and self._pending_count > 0:
            if _asyncio.get_event_loop().time() > deadline:
                logger.warning(f"TTS drain timeout: {self._pending_count} frames remaining")
                return False
