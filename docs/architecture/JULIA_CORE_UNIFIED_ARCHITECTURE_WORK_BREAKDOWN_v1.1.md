...
U1  Architecture Governance Cleanup
        ↓
P0  Production Reality Audit (READ-ONLY)
        ↓
...
### Commit 1 — canonical architecture freeze
...
After Commit 1, calculate:

- `freeze_commit`;
- Git blob SHA;
- SHA-256 of the canonical architecture file.

### Commit 2 — governance evidence record
...
```json
{
  ...
  "version": "1.0",
  "status": "FROZEN",
  "freeze_commit": "<commit sha from Commit 1>",
  "git_blob_sha": "<blob sha from Commit 1>",
...
P0 exists to ensure C-00 through C-12 are grounded in the real production call graph rather than assumptions derived from documentation.
...
**Exit:** P0 PASS — factual input ready for C-00.
...
## Batch C — Production Reality Audit
...
## Batch H — Production Convergence + Staged Historical Migration
...
## U0 recommended commits

### Commit 1

```text
U0: reconcile unified architecture amendments
```

Contains U0-T01 through U0-T12 as one coherent architecture-review unit.

### Commit 2

```text
U0: freeze canonical architecture and index
```
...
This is the architecture freeze commit whose SHA/blob/hash are recorded.

### Commit 3

```text
U0: record architecture freeze evidence
```

Creates the independent `ARCHITECTURE_FREEZE_RECORD_v1.0.json`.
...
# 31. Owner / RACI and Repository Write Boundaries
...
| Historical migration Core side | Claude | legacy data-source adapter only |
...
# 32. Definition of Done for Every Contract
...
# 33. Definition of Done for Production Convergence
...
# 34. HOLD List During Consolidation
...
Do not start C-00 before U0, U1, and P0 PASS.

---

# 36. Program Completion State
