...
class JuliaSession:
    ...
    # ── R0.3b Market Context Compatibility Seam (P3-CC I2-B) ─────────────
    # Pre-cognitive market semantic selection is RETIRED. Context OS
    # (context_execution_runtime.prepare — read-only for I2-B) still invokes
    # this seam, so the method remains mechanically callable, but it is
    # NON-SEMANTIC / NON-EXECUTING / NON-PREFETCHING and always returns empty.
    # Julia cognition is the sole authority for selecting any governed
    # market/research capability. The former chain
    # (_is_market_intent → WorkflowRouter.route → MarketBriefIntentResolver →
    #  resolve_market_intent → MarketBriefPipeline → formatted injection)
    # has zero production callers and no longer exists in this module.

    def _resolve_market_context(self, text: str) -> str:
        """P3-CC I2-B compatibility seam — always empty.

        Does NOT inspect user text, does NOT classify market intent, does NOT
        route through WorkflowRouter / MarketBriefIntentResolver /
        resolve_market_intent, does NOT execute capabilities, does NOT load
        Market pipeline results, and does NOT inject any model-visible market
        prefetch. Model-visible market context flows only from a governed
        capability that Julia cognition explicitly selects.
        """
        return ""
